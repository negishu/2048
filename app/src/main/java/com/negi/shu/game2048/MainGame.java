...
public class MainGame {
    ...
    private void spawnTile(Tile tile) {
        ...
        aGrid.startAnimation(tile.getX(), tile.getY(), SPAWN_ANIMATION, SPAWN_ANIMATION_TIME, MOVE_ANIMATION_TIME, null); //Direction: -1 = EXPANDING
    }
    ...
    public void move(int direction) {
        ...
        prepareUndoState();
        Cell aCell = getACell(direction);
        List<Integer> traversalsX = buildTraversalsX(aCell);
        List<Integer> traversalsY = buildTraversalsY(aCell);
        boolean moved = false;
        prepareTiles();
        for (int xx : traversalsX) {
            for (int yy : traversalsY) {
                ...
                if (tile != null) {
                    Cell[] positions = findFarthestPosition(cell, aCell);
                    Tile next = grid.getCellContent(positions[1]);
                    if (next != null && next.getValue() == tile.getValue() && next.getMergedFrom() == null) {
                        int nNewValue = tile.getValue() * 2;
                        Tile merged = new Tile(positions[1], nNewValue);
                        Tile[] temp = {tile, next};
    ...
    private Cell getACell(int direction) {
        Cell[] map = {
                new Cell(0, -1), // up
                new Cell(1, 0),  // right
                new Cell(0, 1),  // down
    ...
    private List<Integer> buildTraversalsX(Cell aCell) {
    ...
    private List<Integer> buildTraversalsY(Cell aCell) {
        List<Integer> traversals = new ArrayList<>();
            ...
            for (int yy = 0; yy < numSquaresY; yy++) {
                ...
                if (tile != null) {
                    for (int direction = 0; direction < 4; direction++) {
                        Cell vector = getACell(direction);
